    ...
    return (
        <Col lg={12} md={12} className='table-responsive invoice-data-table'>
            <Table bordered >
                ...
                <tbody>
                    {state.data.map((each, index) => (<tr key={index} className="table-inputs">
                        <td>{index + 1}</td>
                        <td><input value={each.item} onInput={(event) => handleDataInp('description', event.target.value, index)} placeholder='item description' /></td>
                        ...
                        <td className="text-end">{state.currency}{each.amount}.00</td>
                        <td>{state.data.length > 1 && <button className='py-0 border-0 bg-transparent' onClick={() => handleDeleteRow(index)}>x</button>}</td>
                    </tr>))}
                </tbody>
            </Table>
